...
class ViPCDataLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            views = self.transform(Image.open(view_path))
            views = views[:3,:,:]
            # print(f"DataLoader - Image loaded and transformed: {views.shape}")
            
            with open(pc_path, 'rb') as f:
                pc = pickle.load(f).astype(np.float32)
            # print(f"DataLoader - GT point cloud loaded: {pc.shape}")
            
            with open(pc_part_path, 'rb') as f:
                pc_part = pickle.load(f).astype(np.float32)
            # print(f"DataLoader - Partial point cloud loaded: {pc_part.shape}")
        ...
        # print(f"DataLoader - Before point padding: pc_part shape: {pc_part.shape}")
        if pc_part.shape[0] < self.pc_input_num:
            pc_part = np.repeat(pc_part, (self.pc_input_num // pc_part.shape[0]) + 1, axis=0)[0:self.pc_input_num]
            # print(f"DataLoader - After point padding: pc_part shape: {pc_part.shape}")
        ...
        # print(f"DataLoader - Before normalization: pc shape: {pc.shape}, pc_part shape: {pc_part.shape}")
        ...
        # print(f"DataLoader - After normalization: pc shape: {pc.shape}, pc_part shape: {pc_part.shape}")
        # print(f"DataLoader - Views shape: {views.shape}")
