...
class MappingNet(nn.Module):
    ...
    def forward(self, x):
        # print(f"MappingNet - Input shape: {x.shape}")
        
        x = F.relu(self.bn1(self.fc1(x).permute(0,2,1)))
        x = x.permute(0,2,1)
        # print(f"MappingNet - After fc1: {x.shape}")
        
        x = F.relu(self.bn2(self.fc2(x).permute(0,2,1)))
        x = x.permute(0,2,1)
        # print(f"MappingNet - After fc2: {x.shape}")
        
        x = F.relu(self.bn3(self.fc3(x).permute(0,2,1)))
        x = x.permute(0,2,1)
        # print(f"MappingNet - After fc3: {x.shape}")
        
        x = F.relu(self.bn4(self.fc4(x).permute(0,2,1)))
        x = x.permute(0,2,1)
        # print(f"MappingNet - Final output shape: {x.shape}")
...
class AXform(nn.Module):
    ...
    def forward(self, x):
        # print(f"AXform - Input shape: {x.shape}")
        
        x_base = x
        x = x.transpose(1, 2).contiguous()
        # print(f"AXform - After transpose: {x.shape}")
        
        x = F.relu(self.bn1(self.conv1(x)))
        # print(f"AXform - After conv1: {x.shape}")
        
        x = F.relu(self.bn2(self.conv2(x)))
        # print(f"AXform - After conv2: {x.shape}")
        
        x = self.conv3(x)
        # print(f"AXform - After conv3: {x.shape}")
       
        x_weights = self.softmax(x)
        # print(f"AXform - Attention weights shape: {x_weights.shape}")
        
        x = torch.bmm(x_weights, x_base)  
        # print(f"AXform - After attention bmm: {x.shape}")
        
        x = x.transpose(1, 2).contiguous()
        x = self.conv4(x)
        # print(f"AXform - After conv4: {x.shape}")
        
        x = x.transpose(1, 2).contiguous()
        # print(f"AXform - Final output shape: {x.shape}")
...
class Decoder_Network(nn.Module):
    ...
    def forward(self, x, x_part):
        # print(f"Decoder - Input shapes: x (features) {x.shape}, x_part {x_part.shape}")
       
        x_partial = x_part.contiguous() # .contiguous() to make fps work
        x_partial = farthest_point_sample(x_partial, 1024)
        # print(f"Decoder - After FPS: x_partial {x_partial.shape}")
        
        x_feat = x
        x_1 = torch.empty(size=(x_part.shape[0], 0, 3)).to(x_part.device)
        # print(f"Decoder - Initial x_1 shape: {x_1.shape}")
        
        x_branch = []
        for i in range(self.num_branch):
            _x_1 = self.pointgen[i](self.featmap[i](x_feat)) 
            # print(f"Decoder - Branch {i} output shape: {_x_1.shape}")
            x_1 = torch.cat((x_1, _x_1), dim=1) 
            x_branch.append(_x_1)

        # print(f"Decoder - After concatenating all branches: x_1 {x_1.shape}")
        x_coarse = torch.cat((x_1, x_partial), dim=1)
        # print(f"Decoder - Final output shape: x_coarse {x_coarse.shape}")
